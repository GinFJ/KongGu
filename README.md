...
<p align="center">
  <img alt="Version" src="https://img.shields.io/badge/version-0.4.2-116c54" />
...
有人缺英方课表时，先到成员身份视图处理，处理完之前他不会出现在空闲名单里。
...
| 视图 | 看什么 |
| :--- | :--- |
| 多人空闲 | 哪些周次、日期、星期、节次有多少人空闲，具体是谁 |
| 成员身份 | 每个成员是否同时导入中方和英方课表，有无缺失或冲突 |
| 文件处理 | 每个 PDF 是否识别成功，识别为谁、哪类课表、多少课程块 |

导出的 Excel 用于实际排班。导出前先处理问题门禁里的异常；有阻断错误时导出按钮不可用，系统不会放行带病结果。
...
点击“修复离线材料”，从安装包内置资源校验并恢复。开发环境可以先用 `npm.cmd run prepare:offline:dev` 跳过 OCR 模型。
...
成员身份视图会标出缺哪一类。导出前处理完，否则他的空闲时间不作数。
...
| 项目 | 状态 | 说明 |
| :--- | :--- | :--- |
| 单元测试 | 已验证 | 121 passed，2 warnings（2026-08-11，0.4.2 工作区） |
...
| 正式安装包 | 未同步 | 已核验安装包为 0.4.0；当前 0.4.2 未构建安装包 |
...
最近一次代码验证（2026-08-08，commit `b7b4977`）：
...
阿谷是空谷的非人虚拟形象，一只从共同空课时间里出发、向着阳光生长的谷物精灵。双叶冠承接青禾，腹部透明种仁代表“青心”，圆润谷粒身体对应青禾最终生长为谷物的意象。

> 青心如禾，向阳而生。

角色母版、头像、应用图标、启动页和宣传材料的使用边界见 [`《阿谷角色与视觉使用规范》`](./docs/品牌/阿谷角色与视觉使用规范.md)。
